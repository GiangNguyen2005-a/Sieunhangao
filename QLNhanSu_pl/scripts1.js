...
const pages = {
    dashboard: `
        <div class="header"> <h1>Chào mừng trở lại, <span>HR Manager!</span></h1> <p class="sub">Hệ thống Quản lý Thông tin Nhân sự</p> </div>
        <div class="actions">
            <div class="action-box lightblue" data-page="ho-so"> <i class="fas fa-users"></i> <h3>Quản lý Hồ sơ NV</h3> <p style="font-size: 13px; color: #555;">Thêm, xem, sửa thông tin.</p> </div>
            <div class="action-box lightgreen" data-page="hop-dong"> <i class="fas fa-file-contract"></i> <h3>Quản lý Hợp đồng</h3> <p style="font-size: 13px; color: #555;">Theo dõi, tạo mới, gia hạn.</p> </div>
            <div class="action-box lightpurple" data-page="thu-tuc"> <i class="fas fa-tasks"></i> <h3>Quản lý Thủ tục</h3> <p style="font-size: 13px; color: #555;">Quyết định, nghỉ việc.</p> </div>
            <div class="action-box lightpink" data-page="khen-thuong-ky-luat"> <i class="fas fa-award"></i> <h3>Khen thưởng & Kỷ luật</h3> <p style="font-size: 13px; color: #555;">Ghi nhận quyết định KTKL.</p> </div>
            <div class="action-box lightorange" data-page="bao-cao"> <i class="fas fa-chart-bar"></i> <h3>Báo cáo Thống kê</h3> <p style="font-size: 13px; color: #555;">Xem báo cáo tổng hợp.</p> </div>
        </div>
    `,
    'ho-so': `
    ...
    'thu-tuc': `
        ...
        <div style="margin-top: 30px;">
            <h3 id="decision-list-header">Danh sách Quyết định</h3>
            <div class="table-responsive-wrapper">
    ...
    'khen-thuong-ky-luat': `
        ...
        <div id="discipline-form-container" class="hidden-form reward-discipline-form">
            ...
            <form id="discipline-form" class="form-area">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label>Ngày xảy ra/Quyết định:</label>
        ...
        <h3 style="margin-top: 30px;">Danh sách Khen thưởng</h3>
        <div class="table-responsive-wrapper">
...
function loadPage(page) {
    ...
    if (pages[page]) {
        ...
        try {
            ...
            else if (page !== 'dashboard') console.warn(`No specific binding function defined for page: ${page}`);
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        reportResultsContainer.innerHTML = `
                            ...
                            <ul>
                                ${Object.entries(data.byMonth).map(([month, stats]) => 
                        ...
                        reportChart = new Chart(reportChartCanvas, {
                            ...
                            data: {
                                ...
                                datasets: [
                                    {
                                        label: 'Nhân viên mới',
                                        data: Object.values(data.byMonth).map(m => m.new),
                                        borderColor: 'rgba(75, 192, 192, 1)',
                                        fill: false
                                    },
                                    {
                                        label: 'Nhân viên nghỉ',
                                        data: Object.values(data.byMonth).map(m => m.terminated),
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!sidebarMenu || !mainContent) {
        console.error("Critical elements missing: sidebarMenu or mainContent not found!");
        return;
    }
    ...
    sidebarMenu.addEventListener('click', (e) => {
        const menuItem = e.target.closest('li[data-page]');
        if (menuItem) {
            const page = menuItem.dataset.page;
            console.log(`Menu item clicked: Navigating to page "${page}"`);
            loadPage(page);
    ...
    mainContent.addEventListener('click', (e) => {
        ...
        if (actionBox) {
            const page = actionBox.dataset.page;
            console.log(`Action box clicked: Navigating to page "${page}"`);
            loadPage(page);

            sidebarMenu.querySelectorAll('li').forEach(item => item.classList.remove('active'));
            const correspondingMenuItem = sidebarMenu.querySelector(`li[data-page="${page}"]`);
