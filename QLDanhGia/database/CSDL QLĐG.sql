DROP TABLE IF EXISTS BaoCaoDanhGia;
DROP TABLE IF EXISTS ChiTietPhanHoiDanhGia;
DROP TABLE IF EXISTS PhieuQuanLyDanhGia;
DROP TABLE IF EXISTS PhieuTuDanhGia;
DROP TABLE IF EXISTS DoiTuongKyDanhGia;
DROP TABLE IF EXISTS KyDanhGia;
-- Quản lý đánh giá 
-- Bảng Kỳ Đánh Giá
CREATE TABLE KyDanhGia (
    MaKyDG VARCHAR(10) PRIMARY KEY,
    TenKyDG VARCHAR(200) NOT NULL,
     ViTriDG VARCHAR(10) NOT NULL,
   ThoiGianApDung DATE NOT NULL, -- Ngày bắt đầu áp dụng (VD: 01/01/2024 cho Quý 1 2024)
    ...
    PhuongPhapDanhGia VARCHAR(100) NULL, -- Có thể tách bảng nếu cần
    MaNV_NguoiTao VARCHAR(10) NULL, -- Nhân viên HR tạo kỳ
    NgayTao DATETIME DEFAULT CURDATE(),
    TrangThai VARCHAR(50) DEFAULT 'Mới tạo', -- Ví dụ: Mới tạo, Đang diễn ra, Đã đóng
...
CREATE TABLE DoiTuongKyDanhGia (
    ...
    TrangThaiTuDanhGia VARCHAR(50) DEFAULT 'Chưa thực hiện', -- Chưa thực hiện, Đã thực hiện
    TrangThaiQLDanhGia VARCHAR(50) DEFAULT 'Chờ đánh giá', -- Chờ đánh giá, Đã đánh giá, Đã phản hồi, Đã xác nhận
...
CREATE TABLE PhieuQuanLyDanhGia (
    ...
    TinhTrang VARCHAR(30) DEFAULT 'Chờ đánh giá',
...
CREATE TABLE ChiTietPhanHoiDanhGia (
    ...
    NoiDung LONGTEXT NOT NULL,  -- Đã sửa: Thay VARCHAR(MAX) bằng LONGTEXT
...
CREATE TABLE BaoCaoDanhGia (
    ...
    LoaiBaoCao VARCHAR(50) NULL, -- VD: Tong hop theo ky, Theo phong ban,...
